...
<p>Re-import data from fresh from drupal.org.</p>

<form enctype="multipart/form-data" method="post">
  <input type="submit" name="submit" value="Import new donations">
  <input type="submit" name="submit" value="Delete all" class="form-submit-button-danger">
</form>
...
if ($_POST['submit'] == 'Delete all') {
  mysqli_query($GLOBALS['connection'], "TRUNCATE `vouchers`") or die (mysqli_error($GLOBALS['connection']));
}

// Insert more.
if ($_POST['submit'] == 'Import new donations') {
  $dom = new Dom;
  ...
  foreach ($contents as $content) {
    ...
    if (count($link)) {
      ...
      // orgs have no brackets.
      else {
        $friendlyName = $nameHtml;
      }
    }

    // No d.o link.
    else {
      preg_match('# from (.+)$#', $html, $matches);
      $friendlyName = $matches[1];
      $doName = '';
      $doUrl = '';
    }

    $key = $doName ?: $friendlyName;

    if (isset($doners[$key])) {
      $doners[$key]['amount'] += $amount;
    }
    else {
      $doners[$key] = [
        'amount' => $amount,
  ...
  foreach ($doners as $doner) {
      ...
      mysqli_query($GLOBALS['connection'], "INSERT INTO `vouchers` (`code`, `banner_id`, `blocks_discount`, `price_discount`, `notes`, `do_username`, `name`)
           VALUES ('{$code}', 0, '${blocks_discount}', '{$price_discount}', 'Imported by the scraper', '{$doner['doName']}', '{$doner['friendlyName']}') ") or die (mysqli_error($GLOBALS['connection']));
  }
