...
if (isset($_REQUEST['select']) && !empty($_REQUEST['select'])) {
	...
	if ($_REQUEST['sel_mode']=='sel4') {
		...
		$max_x = $b_row['grid_width']*$banner_data['BLK_WIDTH'];
		$max_y = $b_row['grid_height']*$banner_data['BLK_HEIGHT'];
...
function hide_block(clicked_block, OffsetX, OffsetY, status) {
	...
	if ((!myblock)) {
		...
		myblocks.innerHTML = myblocks.innerHTML+"<span id='block"+clicked_block+"' style='top: "+(OffsetY+pixelimg.offsetTop)+"; left: "+(OffsetX+pixelimg.offsetLeft)+";' onclick='change_block_state("+OffsetX+", "+OffsetY+");' onmousemove='show_pointer2(this, event)' ><img src='get_block_image.php?BID=<?php echo $BID;?>&image_name=grid_block'></span>";
...
<input style="cursor: pointer;outline:none;border:none;" id="pixelimg" <?php if (USE_AJAX=='YES') { ?> onmousemove="show_pointer(event)" onclick="if (select_pixels(event)) return false;" <?php } ?> type="image" name="map" value='Select Pixels.' style="width:<?php echo $b_row['grid_width']*$banner_data['BLK_WIDTH']; ?>px;height:<?php echo $b_row['grid_height']*$banner_data['BLK_HEIGHT']; ?>;border:none;outline:none;" src="show_selection.php?BID=<?php echo $BID;?>&gud=<?php echo time();?>" >
